# streamlit_app.py
# TRUST DEMO — Map + Attack Injection + Persona-Aware Incidents + EU Governance/Audit (Charts++)
# ----------------------------------------------------------------------------------------------
...
class CFG:
    ...
    model_version: str = "demo-0.3.2"   # surfaced in governance/audit
...
def _init_gov_state():
    ...
    ss.setdefault("gov_controls", {
        ...
        "transparency_cards": True,
        "traceability_audit": True,
        "data_minimization": True,
        "privacy_by_design": True,
        "security_transport": True,
...
if help_mode:
    st.info("Use the left panel to inject attacks. The **map** shows live risk. "
            "Open **Incidents** for persona-aware explanations; **Insights** gives trends & compliance metrics.")
...
# ---------- Governance (EU AI Act / GDPR / NIS2) — MAJOR UPGRADE WITH CHARTS
with tab_governance:
    _init_gov_state()
    ss = st.session_state
    ...
    # Helper: compute transparency/oversight scores (0–100) from toggles + data
    def compute_scores():
        controls = ss.gov_controls
        di = ss.gov_data_inventory
        al = pd.DataFrame(ss.audit_log)
        ...
        # boost if no PII in inventory
        pii_ratio = 0.0
        if not di.empty and "PII" in di.columns:
            pii_ratio = (di["PII"].astype(str).str.lower() == "yes").mean()
        data_transparency += 25 * (1 - pii_ratio)

        model_transparency = 0
        model_transparency += 35 if controls.get("transparency_cards") else 0
        model_transparency += 35 if controls.get("robustness_eval") else 0
        model_transparency += 30 if controls.get("lifecycle_mgmt") else 0

        oversight = 50 if controls.get("human_oversight") else 0
        oversight += 50 if not al.empty else 0  # decisions logged

        robustness = 0
        robustness += 50 if controls.get("robustness_eval") else 0
        robustness += 50 if controls.get("lifecycle_mgmt") else 0

        traceability = 0
        traceability += 50 if controls.get("traceability_audit") else 0
        traceability += 50 if not al.empty else 0

        privacy = 0
        privacy += 50 if controls.get("privacy_by_design") else 0
        privacy += 50 if ss.get("gov_pseudonymization", False) else 0

        return {
            "Data Transparency": int(data_transparency),
            "Model Transparency": int(model_transparency),
            "Human Oversight": int(oversight),
            "Robustness": int(robustness),
            "Traceability": int(traceability),
            "Privacy": int(privacy),
            "PII Ratio": float(pii_ratio),
        }
    ...
    # === Overview (Radar + coverage + timeline)
    with subtabs[0]:
        c_top1, c_top2, c_top3, c_top4 = st.columns(4)
        c_top1.metric("Model version", CFG.model_version)
        di = pd.DataFrame(ss.incidents) if ss.incidents else pd.DataFrame()
        c_top2.metric("Incidents (session)", 0 if di.empty else len(di))
        al_df = pd.DataFrame(ss.audit_log) if ss.audit_log else pd.DataFrame()
        c_top3.metric("Decisions logged", 0 if al_df.empty else len(al_df))
        c_top4.metric("Controls enabled", sum(bool(v) for v in ss.gov_controls.values()))

        colA, colB = st.columns([2,1], gap="large")

        with colA:
            # Radar (spider) chart
            categories = ["Data Transparency","Model Transparency","Human Oversight","Robustness","Traceability","Privacy"]
            values = [scores[k] for k in categories]
            fig_radar = go.Figure()
            fig_radar.add_trace(go.Scatterpolar(r=values, theta=categories, fill="toself", name="Score"))
        ...
        with colB:
            # Controls coverage
            enabled = sum(1 for v in ss.gov_controls.values() if v)
            disabled = len(ss.gov_controls) - enabled
            fig_cov = px.bar(pd.DataFrame({"Status":["Enabled","Disabled"],"Count":[enabled,disabled]}),
                             x="Status", y="Count", title="Controls coverage")
        ...
        if not al_df.empty:
            tl = al_df.copy()
            tl["minute"] = pd.to_datetime(tl["decided_at"]).dt.floor("T")
            line = tl.groupby("minute").size().reset_index(name="decisions")
            st.plotly_chart(px.line(line, x="minute", y="decisions", markers=True, title="Decision timeline"),
                            use_container_width=True)
        else:
            st.info("No decisions yet. Confirm or dismiss an incident to populate this chart.")

    # === Controls & Roles (heatmap)
    with subtabs[1]:
        ...
        rm = ss.gov_role_matrix.copy()
        bool_cols = [c for c in rm.columns if c != "Role"]
        mat = rm[bool_cols].astype(int)
        fig_perm = px.imshow(mat, text_auto=True, aspect="auto", x=bool_cols, y=rm["Role"], title="Permissions (1=yes)")
        st.plotly_chart(fig_perm, use_container_width=True)

    # === Data & Privacy (donuts + retention)
    with subtabs[2]:
        ...
        with c3:
            lawful_basis = st.selectbox("GDPR lawful basis (deployment)",
                                        ["— Demo —", "Legitimate interests (Art.6.1.f)", "Public interest (Art.6.1.e)", "Contract (Art.6.1.b)"], index=0)

        # Data transparency donut (PII vs non-PII)
        dii = ss.gov_data_inventory.copy()
        if not dii.empty and "PII" in dii.columns:
            pie_df = dii["PII"].astype(str).str.title().value_counts().reset_index()
            pie_df.columns = ["PII", "Datasets"]
            fig_pie = px.pie(pie_df, names="PII", values="Datasets", hole=0.5, title="PII vs Non-PII in Data Inventory")
            st.plotly_chart(fig_pie, use_container_width=True)

        # Retention vs recommendation
        rec_df = pd.DataFrame([
        ...
        fig_ret = go.Figure(data=[
            go.Bar(name="Configured", x=rec_df["Artifact"], y=rec_df["Days"]),
...
"""
    ...
    # === Model & Datasets (transparency charts + Sankey)
    with subtabs[3]:
        st.subheader("Model Transparency & Dataset Cards")

        # Model transparency gauge substitute (bar)
        mt = scores["Model Transparency"]
        dt = scores["Data Transparency"]
        cA, cB = st.columns(2)
        with cA:
            st.plotly_chart(px.bar(pd.DataFrame({"Metric":["Model Transparency"],"Score":[mt]}),
                                   x="Metric", y="Score", range_y=[0,100], title="Model Transparency (0–100)"),
                            use_container_width=True)
        with cB:
            st.plotly_chart(px.bar(pd.DataFrame({"Metric":["Data Transparency"],"Score":[dt]}),
                                   x="Metric", y="Score", range_y=[0,100], title="Data Transparency (0–100)"),
                            use_container_width=True)

        # Data lineage Sankey
        st.markdown("#### Data Lineage (demo)")
        nodes = ["Devices", "Telemetry (synthetic)", "Anomaly Engine", "Incidents", "Exports (JSON/MD)", "Audit Log"]
        node_map = {n:i for i,n in enumerate(nodes)}
        links = dict(
            source=[node_map["Devices"], node_map["Telemetry (synthetic)"], node_map["Anomaly Engine"], node_map["Anomaly Engine"], node_map["Incidents"]],
            target=[node_map["Telemetry (synthetic)"], node_map["Anomaly Engine"], node_map["Incidents"], node_map["Exports (JSON/MD)"], node_map["Audit Log"]],
        ...
        fig_sankey.update_layout(title_text="Data Flow: Sources → Processing → Outputs", font_size=12)
...
"""
    ...
    with subtabs[5]:
        ...
        with c1:
            ...
            st.dataframe(di_all, use_container_width=True, height=250)
        with c2:
    ...
    with subtabs[6]:
        ...
        c1, c2, c3 = st.columns(3)
        with c1:
            ss.gov_log_level = st.selectbox("Log level", ["Debug","Info","Warning","Error"],
                                            index=["Debug","Info","Warning","Error"].index(ss.gov_log_level))
        with c2:
            ss.gov_hash_algo = st.selectbox("Evidence hash", ["SHA-256","SHA-512"],
                                            index=0 if ss.gov_hash_algo=="SHA-256" else 1)
        with c3:
            _ = st.slider("Default incident threshold (reference)", 0.30, 0.95, float(CFG.threshold), 0.01)
        st.write("Changes here are reflected in the **DPIA**, **Model Card**, and evidence context.")
