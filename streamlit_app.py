...
class Config:
    ...
    threshold: float = 0.6  # alert threshold on anomaly risk
...
CFG = Config(
    frac_types={"AMR": 0.32, "Truck": 0.32, "Sensor": 0.22, "Gateway": 0.14}
)
...
def inject_ux():
    ...
    <style>
      ...
      .badge .dot {{ width:.55rem; height:.55rem; border-radius:999px; display:inline-block; }}
      .dot.ok {{ background:{COLOR["ok"]}; }}
      .dot.warn {{ background:{COLOR["warn"]}; }}
      .dot.danger {{ background:{COLOR["danger"]}; }}
      .subtle {{ color:{COLOR["muted"]}; }}
      .kpi-note {{ color:{COLOR["muted"]}; font-size:.84rem; margin-top:-.35rem; }}
...
def small_line_chart(df, x, y, title, height=220, key=None):
...
def move_device(dev):
    # Small random walk, AMR & Truck move more than others
...
def meters_to_latlon_offset(m):
    # naive: 1 degree lat ~ 111km, 1 degree lon ~ 55km at Sundsvall latitude (~62°N)
    return (m/111000.0, m/55000.0)

def simulate_tick(dev, t, scenario, centers):
    """
    Returns a dict of measurements for a single device at tick t.
    Features are intentionally realistic-ish for industrial wireless.
    """
    # Baseline (normal)
    ...
    # Wi-Fi / attach churn indicators (for breach)
    deauth_rate = float(np.clip(np.random.normal(0.3, 0.6), 0, 8))
    assoc_churn = float(np.clip(np.random.normal(0.2, 0.5), 0, 10))
    rogue_rssi_gap = float(np.clip(np.random.normal(-2, 4), -30, 30))  # rogue stronger than legit if >0

    # Update movement and sequence
    move_device(dev)
    st.session_state.seq_counter[dev["device_id"]] += 1
    seq_no = st.session_state.seq_counter[dev["device_id"]]

    # Attack overlays within radius
    ...
    if scenario == "Jamming" and within(centers["jammer"], CFG.jammer_radius_m):
        # Broadband noise → lower SNR/SINR, higher noise/loss/latency/jitter, higher BLER-ish
        delta = np.random.uniform(8, 18)
        snr -= delta
        sinr -= delta*0.8
        noise += np.random.uniform(6, 12)
        loss += np.random.uniform(8, 25)
        latency += np.random.uniform(40, 120)
        jitter += np.random.uniform(10, 30)
        is_attack = 1
        attack_type = "Jamming"

    if scenario == "Access Breach" and within(centers["rogue"], CFG.breach_radius_m):
        # Evil twin / rogue base-station lure
        rogue_rssi_gap = float(np.random.uniform(6, 22))  # rogue stronger than legit
        deauth_rate += np.random.uniform(1.0, 4.5)
        assoc_churn += np.random.uniform(1.0, 5.0)
        # connection quality may be flaky
        loss += np.random.uniform(2, 10)
        latency += np.random.uniform(10, 60)
        is_attack = 1
        attack_type = "Access Breach"

    if scenario == "GPS Spoofing" and dev["type"] in ("AMR","Truck") and within(centers["spoofer"], CFG.spoof_radius_m):
        # Fake GNSS → position jumps, HDOP strange, sat count odd
        pos_err += np.random.uniform(20, 80)
        hdop += np.random.uniform(1.0, 3.5)
        sat_ct = int(max(4, sat_ct + np.random.randint(-6, 6)))
        is_attack = 1
        attack_type = "GPS Spoofing"

    if scenario == "Data Tamper" and within(centers["tamper"], CFG.breach_radius_m):
        # Integrity anomalies: repeated seq, HMAC fail, schema issues
        if np.random.rand()<0.5:
            st.session_state.seq_counter[dev["device_id"]] -= 1  # simulate duplicate / replay
        hmac_fail += np.random.uniform(0.2, 0.6)
        schema_violation += np.random.uniform(0.2, 0.5)
        dup_ratio += np.random.uniform(0.2, 0.6)
        is_attack = 1
        attack_type = "Data Tamper"
...
def make_centers():
    ...
    d1 = meters_to_latlon_offset(60)
    d2 = meters_to_latlon_offset(120)
    return {
...
def synthesize_dataset(n_ticks=200, scenario_mix=None, progress_cb=None, pct_start=0, pct_end=70):
    ...
    D=[]
    devices = [d.copy() for d in st.session_state.devices]
    # Normalize mix
    keys = list(scenario_mix.keys())
    probs = np.array([scenario_mix[k] for k in keys])
...
def fit_binary(X_tr, y_tr, X_val, y_val):
    ...
    model = LGBMClassifier(
        ...
        class_weight=cw, random_state=CFG.rng_seed
    )
    model.fit(X_tr, y_tr, eval_set=[(X_val, y_val)], verbose=False)
    return model

def fit_type(X_tr, y_tr):
    # Only on attack points
    model = LGBMClassifier(
...
def conformal_calibration(probs_cal: np.ndarray, y_cal: np.ndarray):
    """Return calibration scores for nonconformity of positive class (attack)."""
    # nonconformity score alpha = 1 - P(y_true)
...
def conformal_pvalue(prob_pos: float, cal_alphas: np.ndarray):
    # For "attack" hypothesis: alpha* = 1 - prob_pos; p = (# alpha_i >= alpha*) / (n+1)
    alpha_star = 1.0 - float(prob_pos)
    n = len(cal_alphas)
    ge = int(np.sum(cal_alphas >= alpha_star))
    return (ge + 1) / (n + 1)
...
def train_models_cached(seed:int=CFG.rng_seed, ticks:int=260) -> Dict[str,Any]:
    ...
    X_train, X_cal, y_train, y_cal = train_test_split(X_tmp, y_tmp, test_size=0.25, random_state=seed, stratify=y_tmp) # 0.6/0.2/0.2
    # Binary model
    ...
    # Global SHAP (optional, small sample)
    global_shap = None
    if HAS_SHAP:
        try:
            ...
            # For binary LightGBM, shap_values can be list [neg, pos]; choose pos
...
def build_enduser_symptoms(inc: dict) -> List[str]:
    ...
    def z(name, thresh=0.8):  # use raw values: compute coarse z from session rolling?
        # Very simple standardization vs. nominal anchors
        anchors = {
            "snr": 22, "sinr_db": 18, "noise_floor_dbm": -92,
            "packet_loss": 1.5, "latency_ms": 28, "jitter_ms": 4.0,
            "pos_error_m": 3.0, "gnss_hdop": 1.4,
            "hmac_fail_rate": 0.05,"schema_violation_rate":0.03,"dup_ratio":0.02,
            "deauth_rate":0.3,"assoc_churn":0.2,"rogue_rssi_gap": -2
        }
        stds = {
            "snr": 4, "sinr_db": 5, "noise_floor_dbm": 3,
            "packet_loss": 1.2, "latency_ms": 10, "jitter_ms": 2,
            "pos_error_m": 2.0, "gnss_hdop": 0.4,
            "hmac_fail_rate": 0.08,"schema_violation_rate":0.05,"dup_ratio":0.04,
            "deauth_rate":0.6,"assoc_churn":0.5,"rogue_rssi_gap": 4
        }
        v = f.get(name, None)
        if v is None: return 0.0
        return (float(v) - anchors[name]) / (stds[name]+1e-6)

    bullets = []
    if z("noise_floor_dbm") > 2.0 or z("snr") < -2.0 or z("sinr_db") < -2.0:
        bullets.append("Signal quality **degraded** (interference likely).")
    if z("packet_loss") > 2.0 or z("jitter_ms") > 2.0 or z("latency_ms") > 2.0:
        bullets.append("**High errors / delay** observed.")
    if z("pos_error_m") > 2.0 or z("gnss_hdop") > 2.0:
        bullets.append("Location reading **unstable**.")
    if z("hmac_fail_rate") > 2.0 or z("schema_violation_rate") > 2.0 or z("dup_ratio") > 2.0:
        bullets.append("Data integrity **issues** detected.")
    if z("deauth_rate") > 2.0 or z("assoc_churn") > 2.0 or f.get("rogue_rssi_gap", -10) > 6:
        bullets.append("Unusual **network (re)auth activity**.")
    if not bullets:
        bullets.append("Behavior is **unusual** compared to normal for this device.")
    return bullets[:3]

def build_anomaly_explanation(inc: dict) -> str:
    # Uses SHAP if available; otherwise shows top deviating features
    fvals = inc.get("features", {})
    out = []
    # plain ranking by magnitude of standardized deviation
    ...
    for k,v in top:
        arrow = "↑" if v>0 else "↓"
        out.append(f"{arrow} **{k}** deviated ({v:+.2f}σ).")
    return " • ".join(out)
...
def fuse_type_probs(ml_probs: Dict[str,float], feats: Dict[str,float]) -> Tuple[str,float,Dict[str,float]]:
    """Combine ML and simple rules for final type & confidence."""
    rules = {"Jamming":0.0, "Access Breach":0.0, "GPS Spoofing":0.0, "Data Tamper":0.0}
    # Jamming heuristic
    if feats.get("snr",30) < 12 or feats.get("noise_floor_dbm",-95) > -85 or feats.get("packet_loss",0) > 8:
        rules["Jamming"] += 0.35
    # Breach heuristic
    if feats.get("rogue_rssi_gap",-10) > 6 or feats.get("deauth_rate",0) > 2.0 or feats.get("assoc_churn",0) > 2.0:
        rules["Access Breach"] += 0.35
    # Spoofing heuristic
    if feats.get("pos_error_m",0) > 15 or feats.get("gnss_hdop",0) > 3.0:
        rules["GPS Spoofing"] += 0.35
    # Tamper heuristic
    if feats.get("hmac_fail_rate",0) > 0.2 or feats.get("schema_violation_rate",0) > 0.2 or feats.get("dup_ratio",0) > 0.2:
        rules["Data Tamper"] += 0.35

    fused = {}
    for k in rules.keys():
        fused[k] = 0.65*ml_probs.get(k,0.0) + 0.35*rules[k]
    label = max(fused.items(), key=lambda kv: kv[1])[0]
    conf = fused[label]
...
def detect_on_row(row: pd.Series) -> Dict[str,Any]:
    ...
    # Type head (only if anomalous-ish)
    ...
    if st.session_state.type_model is not None:
        ...
        # Make dict excluding Normal if present
        for i,lab in enumerate(labels):
            if lab=="Normal": continue
            ml_probs[lab] = float(tp[i] if i < len(tp) else 0.0)
    # Fuse
...
5. **Open Incidents** tab for plain explanations & what-to-do.  
...
st.markdown(
    ...
    <div class="panel">
      ...
      <div style="margin-top:.4rem; display:flex; gap:12px; flex-wrap:wrap;">
...
def run_one_tick():
    ...
    for dev in st.session_state.devices:
        rec = simulate_tick(dev, t, scenario, centers)
        recs.append(rec)
...
with tab_overview:
    # Map
    df_map = pd.DataFrame(list(st.session_state.fleet_records)) if st.session_state.fleet_records else pd.DataFrame(st.session_state.devices)
    if len(df_map)==0:
        st.info("No data yet. Train and start streaming.", icon="ℹ️")
    else:
        # Take last seen per device
        df_last = df_map.sort_values("tick").groupby("device_id").tail(1).reset_index(drop=True)
        if ready:
            # If model ready, calculate risk for coloring (safe fallback)
            risks=[]
        ...
        # Layers
        color = "[min(255, int(risk*255)), int((1.0-risk)*180), 40]"
        dev_layer = pdk.Layer(
            "ScatterplotLayer",
            data=df_last,
        ...
        tooltip = {
            "html": "<b>{device_id}</b> · {type}<br/>Risk: {risk}<br/>SNR: {snr} dB · Loss: {packet_loss}%<br/><i>Move mouse for more</i>",
            "style": {"backgroundColor": "rgba(255,255,255,.95)", "color":"#111"}
        }
        st.pydeck_chart(pdk.Deck(layers=[dev_layer, warn_layer, text_layer], initial_view_state=view, tooltip=tooltip), use_container_width=True)
...
def render_device_inspector(inc:Dict[str,Any], base_key:str):
    # Risk gauge (textual) + local SHAP + tiny history
...
def render_incident_card(inc:Dict[str,Any], role:str, scope:str):
    ...
    if role == "End User":
        ...
        for line in build_enduser_symptoms(inc):
            st.markdown(f"- {line}")
        ...
        with st.expander("Technical details"):
            st.markdown("**Why anomalous (model view):**")
            st.markdown(build_anomaly_explanation(inc))
            st.markdown("**Why this type (model view):**")
...
# ----------------------- Incidents tab -----------------------
with tab_inc:
    if require_model_banner():
        st.stop()
    all_inc = list(reversed(st.session_state.incidents))  # newest first
    if not all_inc:
...
with tab_insight:
    ...
    with k5:
        # fleet risk = mean prob among last window
        fr = pd.DataFrame(list(st.session_state.fleet_records))
        fleet_risk = 0.0
    ...
    for k,v in ATTACK_GLOSSARY.items():
        rows.append({
            "Attack type": v["name"], "What it is": v["what"],
            "Typical signs": v["symptoms"], "Why it matters": v["impact"], "What to do": v["action"]
        })
    st.dataframe(pd.DataFrame(rows), use_container_width=True, hide_index=True, height=220, key=f"gloss_{st.session_state.ui_nonce}")

# ----------------------- Governance tab -----------------------
with tab_gov:
    if not ready:
        st.info("Train the model to populate governance artifacts (metrics, features, calibration).", icon="ℹ️")
